...
public Student(String name, int age, String studySubject) {
	this.name = name;
	this.age = age;
	this.studySubject = studySubject;   //all studysubjects are written in camelCase
}
public static void numSub(Student s) {
	char[] arr=s.getStudySubject().toCharArray();
	int count=0;
	for(int i=0;i<arr.length;i++) {
		if(Character.isUpperCase(arr[i])) {   //new subject= uppercase letter
			count++;
		}
	}
...
public void setAge(int age) {
	try {
		if(age>20 && age<120) {
			...
			System.out.println("Age is not in valid range "+ex);
			ex.printStackTrace();
		}
	}
